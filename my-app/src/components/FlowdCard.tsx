...
export function FlowdCard({ card, onUpdate, onDelete, onAddToBoard, onChat, onClick, isModal }: FlowdCardProps) {
  ...
  if (card.metadata?.isAboutFlowd) {
    return (
      ...
      >
        <h4 className="font-semibold text-xl mb-4 leading-snug shrink-0 text-white tracking-wide">
          {card.title}
        </h4>
        <div className={`relative ${!isModal ? 'flex-1 overflow-hidden' : ''}`}>
          <div className={`text-[15px] leading-relaxed text-[#b3b3b3] space-y-4 ${!isModal ? 'line-clamp-4' : ''}`}>
            {card.content.split('\n\n').map((paragraph, idx) => (
  ...
  if (card.category === 'open_question') {
    return (
      ...
      >
        ...
        {/* Title */}
        <h4 
          className={`font-medium text-base leading-snug mb-4 ${!isModal ? 'line-clamp-6' : ''}`}
        ...
        {card.answer && (
          <div className="mt-4 p-4 rounded-xl bg-white/50">
            ...
            <p className={`text-sm leading-relaxed italic ${!isModal ? 'line-clamp-4' : ''}`} style={{ color: config.textColor }}>
              {card.answer}
            </p>
          </div>
        )}
          ...
          <ul className={`space-y-5 mb-2 relative ${!isModal ? 'overflow-y-auto flex-1' : ''}`}>
            {items.map((item, idx) => (
              ...
              >
                ...
                <span 
                  className={`text-[19px] font-medium tracking-tight leading-snug transition-all ${checkedItems[idx] ? 'line-through decoration-2 decoration-white/40' : ''}`}
                  style={{ 
  ...
  if (card.category === 'decided') {
    return (
      ...
      >
        ...
        <div 
          className={`text-lg leading-relaxed mb-6 font-medium relative ${!isModal ? 'flex-1 overflow-hidden' : ''}`}
          style={{ color: config.textColor }}
        >
  ...
  if (card.category === 'meeting') {
    return (
      ...
      >
        ...
        <div className={`space-y-2 text-sm ${!isModal ? 'overflow-hidden flex-1' : ''}`} style={{ color: config.textColor, opacity: 0.9 }}>
  ...
  return (
    ...
    >
      ...
      {/* Title */}
      <h4 
        className={`font-medium text-sm mt-2 leading-snug shrink-0 ${!isModal ? 'line-clamp-3' : ''}`}
        style={{ color: config.textColor }}
      >
        {card.title}
      </h4>
      ...
      {(isExpanded || card.content.length < 80) && card.content !== card.title && (
        <div className={`relative mt-2 ${!isModal ? 'flex-1 overflow-hidden' : ''}`}>
          <p 
            className={`text-xs leading-relaxed opacity-80 ${!isModal ? 'line-clamp-10' : ''}`}
            style={{ color: config.textColor }}
          >
            {card.content}
          </p>
